...
try:
    ...
    from PySide6.QtWidgets import QLineEdit, QInputDialog, QTableWidgetItem
    from PySide6.QtGui import QIcon
except:
    from PyQt5.QtCore import Qt, QSize, QPoint, QThread, QTranslator, QCoreApplication, QLocale
    from PyQt5.QtWidgets import QLineEdit, QInputDialog, QTableWidgetItem
    from PyQt5.QtGui import QIcon

from persepolis.constants import OS
from persepolis.gui.torrent_progress_ui import TorrentProgressWindow_Ui
...
class TorrentProgressWindow(TorrentProgressWindow_Ui):
    ...
    def updateFilesTable(self, torrent_files_list):
        self.files_table.clear()
        files_table_header_labels = [QCoreApplication.translate("torrentprogresswindow_ui_tr", "File Name"),
                                     QCoreApplication.translate("torrentprogresswindow_ui_tr", "Percentage"),
                                     QCoreApplication.translate("torrentprogresswindow_ui_tr", "File Size/Downloaded Size")]
        self.files_table.setHorizontalHeaderLabels(files_table_header_labels)

        for file_list in torrent_files_list:
            self.files_table.insertRow(0)

            # file_name
            file_name = file_list[0]
            downloaded_size = file_list[1]
            ...
            # insert download percentage
            percentage = downloaded_size * 100 // file_size
            item = QTableWidgetItem(str(percentage))
            # centers horizontally and vertically
            item.setTextAlignment(Qt.AlignCenter)

            self.files_table.setItem(0, 1, item)

            # inser downloded size and file size
            downloaded_size_h, downloaded_size_u = humanReadableSize(downloaded_size)
            file_size_h, file_size_u = humanReadableSize(file_size)
            str_ = str(downloaded_size_h) + ' ' + downloaded_size_u + '/' + str(file_size_h) + ' ' + file_size_u
            item = QTableWidgetItem(str_)
            # centers horizontally and vertically
            item.setTextAlignment(Qt.AlignCenter)

            self.files_table.setItem(0, 2, item)
