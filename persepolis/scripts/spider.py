...
def spider(add_link_dictionary: dict[str, Any]) -> tuple[str, str]:
    ...
    if ip:
        ...
        if proxy_type == 'socks5':  # noqa: SIM108
...
def queueSpider(add_link_dictionary: dict[str, Any]) -> str:
    filename = addLinkSpider(add_link_dictionary)[0]

    return filename  # noqa: RET504


def addLinkSpider(add_link_dictionary: dict[str, Any]) -> tuple[str, str]:
