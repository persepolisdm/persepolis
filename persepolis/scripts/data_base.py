...
class TempDB:
    ...
    def lockCursor(self) -> None:
        while self.lock:
            rand_float = random.uniform(0, 0.5)  # noqa: S311
            sleep(rand_float)
    ...
    def updateSingleTable(self, dict_: dict[str, Any]) -> None:
        # lock data base
        self.lockCursor()
    ...
    def updateQueueTable(self, dict_: dict[str, Any]) -> None:
        # lock data base
        self.lockCursor()
    ...
    def returnGid(self, gid: str) -> dict[str, str]:
        # lock data base
        self.lockCursor()
        ...
        return {'shutdown': str(tuple_[0]), 'status': tuple_[1]}

    # This method returns values of columns for specific category
    def returnCategory(self, category: str) -> dict[str, str]:
        # lock data base
        self.lockCursor()
...
class PluginsDB:
    ...
    def insertInPluginsTable(self, list_: list[dict[str, str]]) -> None:
        # lock data base
        self.lockCursor()
...
class PersepolisDB:
    ...
    def insertInCategoryTable(self, dict_: dict[str, Any]) -> None:
        # lock data base
        self.lockCursor()
    ...
    def insertInDownloadTable(self, list_: list[dict[str, Any]]) -> None:
        # lock data base
        self.lockCursor()
    ...
    def insertInAddLinkTable(self, list_: list[dict[str, str]]) -> None:
        # lock data base
        self.lockCursor()
    ...
    def insertInVideoFinderTable(self, list_: list[dict[str, str]]) -> None:
        # lock data base
        self.lockCursor()
    ...
    def searchGidInVideoFinderTable(self, gid: str) -> dict[str, str]:
        # lock data base
        self.lockCursor()

        self.persepolis_db_cursor.execute(
            f"""SELECT * FROM video_finder_db_table WHERE audio_gid = '{gid!s}' OR video_gid = '{gid!s}'"""
        )
    ...
    def searchGidInDownloadTable(self, gid: str) -> dict[str, str]:
        # lock data base
        self.lockCursor()

        self.persepolis_db_cursor.execute(f"""SELECT * FROM download_db_table WHERE gid = '{gid!s}'""")
    ...
    def returnItemsInDownloadTable(self, category: str | None = None) -> dict[str, dict]:
        # lock data base
        self.lockCursor()
    ...
    def searchGidInAddLinkTable(self, gid: str) -> dict[str, str] | None:
        ...
        self.persepolis_db_cursor.execute(f"""SELECT * FROM addlink_db_table WHERE gid = '{gid!s}'""")
        list_ = self.persepolis_db_cursor.fetchall()
    ...
    # '*' for category, cause that method returns all items.
    def returnItemsInAddLinkTable(self, category: str | None = None) -> dict[str, dict]:
        # lock data base
        self.lockCursor()
    ...
    def updateDownloadTable(self, list_: list[dict[str, Any]]) -> None:
        # lock data base
        self.lockCursor()
        ...
        for dict_ in list_:
            ...
            # update data base if value for the keys is not None
            self.persepolis_db_cursor.execute(
                '''UPDATE download_db_table SET   file_name = coalesce(:file_name, file_name),
                                                                                    status = coalesce(:status, status),
    ...
    def updateCategoryTable(self, list_: list[dict[str, Any]]) -> None:
        # lock data base
        self.lockCursor()
        ...
        for dict_ in list_:
            ...
            # update data base if value for the keys is not None
            self.persepolis_db_cursor.execute(
                '''UPDATE category_db_table SET   start_time_enable = coalesce(:start_time_enable, start_time_enable),
    ...
    def updateAddLinkTable(self, list_: list[dict[str, Any]]) -> None:
        # lock data base
        self.lockCursor()
        ...
        for dict_ in list_:
            ...
            # update data base if value for the keys is not None
            self.persepolis_db_cursor.execute(
                '''UPDATE addlink_db_table SET out = coalesce(:out, out),
                                                                                start_time = coalesce(:start_time, start_time),
                                                                                end_time = coalesce(:end_time, end_time),
                                                                                link = coalesce(:link, link),
                                                                                ip = coalesce(:ip, ip),
    ...
    def updateVideoFinderTable(self, list_: list[dict[str, Any]]) -> None:
        # lock data base
        self.lockCursor()
        ...
        for dictionary in list_:
            ...
            if dictionary['video_gid']:
                # update data base if value for the keys is not None
                self.persepolis_db_cursor.execute(
                    '''UPDATE video_finder_db_table SET video_completed = coalesce(:video_completed, video_completed),
    ...
    def setDefaultGidInAddlinkTable(
        self, gid: str, start_time: bool = False, end_time: bool = False, after_download: bool = False
    ) -> None:
        # lock data base
        self.lockCursor()
    ...
    def searchCategoryInCategoryTable(self, category: str) -> dict[str, str]:
        # lock data base
        self.lockCursor()

        self.persepolis_db_cursor.execute(f"""SELECT * FROM category_db_table WHERE category = '{category!s}'""")
        list_ = self.persepolis_db_cursor.fetchall()
        ...
        # create a dictionary from results and return
        return {
            'category': tuple_[0],
    ...
    def categoriesList(self) -> list[str]:
        ...
        queues_list = [tuple_[0] for tuple_ in rows]
    ...
    def findActiveDownloads(self, category: str | None = None) -> list[str]:
        # lock data base
        self.lockCursor()

        # find download items is download_db_table with status = "downloading" or "waiting" or paused or scheduled
        if category:
            self.persepolis_db_cursor.execute(f'''SELECT gid FROM download_db_table WHERE (category = '{category!s}') AND (status = 'downloading' OR status = 'waiting'
        ...
        gid_list = [result_tuple[0] for result_tuple in result]
    ...
    def returnVideoFinderGids(self) -> tuple[list[str], list[str], list[str]]:
        # lock data base
        self.lockCursor()
    ...
    def deleteCategory(self, category: str) -> None:
        ...
        self.persepolis_db_cursor.execute(f"""DELETE FROM category_db_table WHERE category = '{category!s}'""")
    ...
    def deleteItemInDownloadTable(self, gid: str, category: str) -> None:
        ...
        self.persepolis_db_cursor.execute(f"""DELETE FROM download_db_table WHERE gid = '{gid!s}'""")
    ...
    def correctDataBase(self) -> None:
        ...
        for units in [['KB', 'KiB'], ['MB', 'MiB'], ['GB', 'GiB']]:
            dict_ = {'old_unit': units[0], 'new_unit': units[1]}
    ...
    def correctDataBaseForVersion410(self) -> None:
        # lock data base
        self.lockCursor()
    ...
    # close connections
    def correctDataBaseForVersion411(self) -> None:
        # lock data base
        self.lockCursor()
        ...
        for gid in gid_list:
            # find download_path
            self.persepolis_db_cursor.execute(f"""SELECT download_path FROM addlink_db_table WHERE gid = '{gid!s}'""")
            ...
            self.persepolis_db_cursor.execute(f'''UPDATE addlink_db_table SET download_path = '{download_path_temp}'
                                                                            WHERE gid = '{gid}' ''')
