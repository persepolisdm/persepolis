...
try:
    ...
    from PySide6.QtCore import QFile, QSettings, Qt, QTextStream
    from PySide6.QtCore import __version__ as QT_VERSION_STR  # noqa: N812
    from PySide6.QtGui import QFont
except:
    from PyQt5 import QtCore, QtWidgets
    from PyQt5.QtCore import QT_VERSION_STR, QFile, QSettings, Qt, QTextStream
...
from persepolis.constants import Os, Version
from persepolis.gui import resources  # noqa: F401
from persepolis.scripts import osCommands
from persepolis.scripts.error_window import ErrorWindow
from persepolis.scripts.useful_tools import determineConfigFolder, osAndDesktopEnvironment
...
global lock_file_validation  # noqa: PLW0604
...
if os_type != Os.WINDOWS:
    ...
    lock_file = '/tmp/persepolis_exec_' + user_name + '.lock'  # noqa: S108
...
class PersepolisApplication(QtWidgets.QApplication):
    ...
    def setPersepolisFont(self, font: str, font_size: int, custom_font: str) -> None:
        # font and font_size
...
parser.add_argument(
    '--parent-window', action='store', nargs=1, help='this switch is used for chrome native messaging in Windows'
)
...
if args.parent_window or unknownargs:
    ...
    message = '{"enable": true, "version": "1.85"}'.encode('utf-8')  # noqa: UP012
...
add_link_dictionary['referer'] = ''.join(args.referer) if args.referer else None

add_link_dictionary['load_cookies'] = ''.join(args.cookie) if args.cookie else None

add_link_dictionary['user_agent'] = ''.join(args.agent) if args.agent else None

add_link_dictionary['header'] = ''.join(args.headers) if args.headers else None

add_link_dictionary['out'] = ''.join(args.name) if args.name else None
