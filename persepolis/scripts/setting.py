...
class PreferencesWindow(Setting_Ui):
    ...
    def iconSizeComboBoxCanged(self, index):
        ...
        if selected_size < 48:
            ...
            icons = ['Archdroid-Red', 'Archdroid-Blue', 'Breeze', 'Breeze-Dark', 'Papirus', 'Papirus-Dark',
                     'Papirus-Light']
    ...
    def okPushButtonPressed(self, button):
        ...
        if icons != self.current_icon:  # it means icons changed
            ...
            for list in [self.parent.logwindow_list, self.parent.about_window_list, self.parent.addlinkwindows_list,
                         self.parent.propertieswindows_list, self.parent.afterdownload_list,
                         self.parent.text_queue_window_list, self.parent.progress_window_list]:
                for window in list:
        ...
        for key in self.first_key_value_dict.keys():
            if self.first_key_value_dict[key] != self.second_key_value_dict[key]:
                if key in ['locale', 'aria2_path', 'download_path_temp', 'download_path', 'custom-font', 'rpc-port',
                           'max-tries', 'retry-wait', 'timeout', 'connections', 'style', 'font', 'font-size',
                           'color-scheme']:
        ...
        if show_message_box:
            ...
            restart_messageBox.setText(QCoreApplication.translate("setting_src_ui_tr",
