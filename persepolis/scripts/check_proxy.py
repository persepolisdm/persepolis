...
def getProxy() -> dict[str, str]:
    ...
    # Detect proxy from GNOME Desktop
        ...
        process = subprocess.run(
            ['gsettings', 'get', 'org.gnome.system.proxy', 'mode'], stdout=subprocess.PIPE, check=False
        )
        mode = re.search(r'manual', process.stdout.decode('utf-8'))
        if mode is not None:
            try:
                process = subprocess.run(
                    ['gsettings', 'get', 'org.gnome.system.proxy.http', 'host'], stdout=subprocess.PIPE, check=False
                )
                proxy['http_proxy_ip'] = re.search(r'\'([\w0-9\.]+)\'', process.stdout.decode('utf-8')).group(1)
                process = subprocess.run(
                    ['gsettings', 'get', 'org.gnome.system.proxy.http', 'port'], stdout=subprocess.PIPE, check=False
                )
            ...
            try:
                process = subprocess.run(
                    ['gsettings', 'get', 'org.gnome.system.proxy.https', 'host'], stdout=subprocess.PIPE, check=False
                )
                proxy['https_proxy_ip'] = re.search(r'\'([\w0-9\.]+)\'', process.stdout.decode('utf-8')).group(1)
                process = subprocess.run(
                    ['gsettings', 'get', 'org.gnome.system.proxy.https', 'port'], stdout=subprocess.PIPE, check=False
                )
            ...
            try:
                process = subprocess.run(
                    ['gsettings', 'get', 'org.gnome.system.proxy.ftp', 'host'], stdout=subprocess.PIPE, check=False
                )
                proxy['ftp_proxy_ip'] = re.search(r'\'([\w0-9\.]+)\'', process.stdout.decode('utf-8')).group(1)
                process = subprocess.run(
                    ['gsettings', 'get', 'org.gnome.system.proxy.ftp', 'port'], stdout=subprocess.PIPE, check=False
                )
            ...
            try:
                process = subprocess.run(
                    ['gsettings', 'get', 'org.gnome.system.proxy.socks', 'host'], stdout=subprocess.PIPE, check=False
                )
    ...
    # if it is windows,mac and other linux desktop
        ...
        try:
            ...
            # if it is Mac OS
                ...
                proxyList = subprocess.run(['scutil', '--proxy'], stdout=subprocess.PIPE, check=False)
